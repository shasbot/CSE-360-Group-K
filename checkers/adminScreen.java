...
public class adminScreen extends javax.swing.JPanel
{
    ...
    {
          for (int j = 0; j < users_list_storage.size(); j++)
            {
                System.out.println(users_list_storage.elementAt(j));
            }
    }

    public void delete_file(String file_name)
    {
		File to_be_deleted = new File("C://users//"+file_name+".txt");
				       ...
				       public void actionPerformed(ActionEvent event)
				         {
						    if(event.getSource() == jButton1)//checks to see if remove button was pushed
						    {
								...
								{
									//userResults.setForeground(Color.red);//sets foreground color to red
									//userResults.setText("Their is nothing to remove");//sets text label at bottom to Their is nothing to remove
						    	}
						    	else
						    	{
